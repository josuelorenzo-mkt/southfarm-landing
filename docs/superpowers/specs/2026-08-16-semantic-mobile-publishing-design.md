...
El handoff leído completo es `docs\SEMIOGANIC_PUBLISHING_HANDOFF_ES.md`, actualizado el 2026-08-14. El checkout actual está en la rama `codex/semiorganic-publishing`; al redactar esta spec el HEAD observado es `50f9bf9`.
...
- `tap_and_wait()` vuelve a tomar un dump antes de seleccionar el nodo, realiza un tap mediante `SafeAdb.tap_bounds()` y espera una revisión fresca que demuestre que el target cambió o desapareció.
...
- `PublicationRunner` persiste checkpoints monotónicos, marca la intención final antes del gesto irreversible y convierte una excepción posterior a la acción final en `review_required`. No se debe reintentar automáticamente `Share`, `Post` o `Upload Short`.
...
- `screen_size`: ancho y alto conocidos cuando el dispositivo pueda reportarlos.
...
7. Verificar que el nodo/contexto de la siguiente pantalla aparece, que el target anterior desaparece o que la transición definida por el adaptador ocurrió.
...
Abrir una app se realiza con el package exacto usando `adb shell monkey -p PACKAGE 1`. Después debe verificarse:
...
- Tomar dump posterior y exigir que el control buscado aparezca o que el estado de la lista cambie.

En particular, el cleanup de TikTok debe reemplazar el actual swipe fijo de `[900,900]` a `[100,900]` por un swipe dentro de los bounds del carrusel de acciones observado. Si el carrusel no tiene bounds o hay más de un contenedor candidato, el cleanup se detiene y no borra nada.
...
Para cleanup de un post de prueba, resolver el menú `vbn`, usar un swipe dinámico dentro del carrusel de acciones, resolver `fq5`, confirmar la eliminación y verificar que el baseline ordenado quedó restaurado. El cleanup no se ejecuta si la identidad no fue verificada, si el diálogo de confirmación es ambiguo o si el estado posterior no prueba la restauración.
...
La comparación de identidad puede tolerar únicamente la representación visual documentada con o sin `@` cuando el contrato del adaptador lo defina; no puede eliminar caracteres ni hacer matching case-insensitive de manera que fusione cuentas. La cuenta configurada como `santilorennzo`, Santiago o cualquier variante equivalente bloqueada debe provocar `FORBIDDEN_ACCOUNT` antes de abrir el selector, hacer un tap o transferir media. Ninguna prueba, dry-run ni cleanup puede tocarla.
...
7. Obtener dump posterior y pasar a verificación.

Si falla el checkpoint, el tap no se ejecuta. Si el checkpoint ya fue persistido, se considera que la intención final existe aunque ADB devuelva un error o la pantalla quede incierta. No se vuelve a tocar el botón. El runner debe terminar en `review_required` cuando la acción pudo ejecutarse y la verificación no decide, manteniendo `final_action_uncertain=true` y evitando reencolar automáticamente.
...
- XML inválido, jerarquía incompleta, bounds invertidos y package inesperado deben fallar cerrado.
...
- El swipe de TikTok debe derivar start/end del bounds del carrusel en dos tamaños de pantalla distintos y producir coordenadas diferentes.
...
- TikTok: `Create` frente a `Create a Story`, identidad pasiva, galería, keyboard-open, visibilidad pública, Post, cover y contador cero, carrusel de cleanup.
...
Cada adaptador debe probar wrong package, cuenta ausente, cuenta ambigua, selector ausente, selector deshabilitado, transición stale, final context faltante y final checkpoint fallido. La prueba debe afirmar que no hubo tap irreversible en esos casos.
...
5. Cada acción física obtiene dump fresco antes y después, y la transición se verifica; el retorno de ADB por sí solo nunca marca éxito.
6. El swipe de cleanup de TikTok usa bounds dinámicos y no conserva `[900,900] → [100,900]` ni otra coordenada de teléfono.
...
8. `Share`, `Post` y `Upload Short` tienen checkpoint previo, una única oportunidad de tap y clasificación `review_required` si la acción final queda incierta.
...
11. La cuenta `santilorennzo`/Santiago es rechazada antes de cualquier acción física y no aparece en ninguna prueba live.
12. El teléfono, la cuenta `@growtech.news`, el backend device identity y los jobs existentes se verifican antes del rollout. No se declara éxito por un job `completed` sin evidencia de perfil/canal.
13. Se ejecutan los tests unitarios, de adaptadores, de integración, build/lint web y pruebas backend especificadas en el handoff sin incorporar los artefactos dirty.
14. Se completa el rollout real autorizado: como máximo dos publicaciones en Instagram sobre `@growtech.news`, usando `MP-V-1.mp4` y `MP-V-2.mp4`, cada una publicada desde la webapp, verificada en el perfil y eliminada después con confirmación de baseline. Si la primera falla o queda `review_required`, no se inicia la segunda.
...
- ciclo obligatorio por cada video: publicar → observar timeline `publishing`/`verifying`/`completed` → verificar el Reel en el perfil correcto → eliminar mediante cleanup explícito → confirmar baseline restaurado.
...
- Se separó el estado actual observado del diseño objetivo: hoy el parser es plano y TikTok aún tiene un swipe fijo; el diseño exige jerarquía y swipe dinámico.
