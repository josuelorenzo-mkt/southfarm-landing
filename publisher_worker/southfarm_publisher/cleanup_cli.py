...
def _load_manifest(path: str) -> dict[str, Any]:
    ...
    required = {"schema", "marker", "job_id", "job_status", "platform", "serial", "android_id", "account", "expected_identity", "baseline", "test_mode"}
    if type(value) is not dict or set(value) != required:
        raise PublisherError("CLEANUP_MANIFEST_INVALID", "Cleanup manifest schema is invalid")
    strings = ("platform", "serial", "android_id", "account", "expected_identity")
...
def execute_cleanup(argv: list[str], *, registry_factory: Callable[..., Any] = AdbDeviceRegistry, adapter_factory: Callable[[str, str], Any] = _adapter, emit: Callable[[dict[str, Any]], None] = print) -> dict[str, Any]:
