...
def _config(env=os.environ) -> tuple[PublisherApiClient, AdbDeviceRegistry, int, set[str]]:
    ...
    ui_source = env.get("SOUTHFARM_UI_SOURCE", "auto").strip().lower()
    if ui_source not in ("service", "uiautomator", "auto"): raise PublisherError("CONFIG_INVALID", "SOUTHFARM_UI_SOURCE must be 'service', 'uiautomator', or 'auto'")
