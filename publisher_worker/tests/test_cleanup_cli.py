...
def manifest(**overrides):
    value = {"schema": 1, "marker": "SOUTHFARM_AUTHORIZED_TEST_POST", "job_id": 7, "job_status": "completed", "platform": "instagram", "serial": "USB-1", "android_id": "android-1", "account": "safe.account", "expected_identity": "new reel", "baseline": ["safe.account", "old reel"], "test_mode": True}
    value.update(overrides); return value


class CleanupCliTests(unittest.TestCase):
    def invoke(self, value, args, *, nodes=None):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = execute_cleanup(["--manifest", str(path), "--platform", "instagram", "--serial", "USB-1", "--android-id", "android-1", "--account", "safe.account", *args], registry_factory=lambda **kwargs: registry, adapter_factory=lambda platform, account: adapter, emit=output.append)
            return result, adapter, device, registry, output
    ...
    def test_dry_run_is_default_and_never_deletes(self):
        result, adapter, device, registry, output = self.invoke(manifest(), [])
    ...
    def test_apply_calls_explicit_cleanup_and_requires_restoration(self):
        result, adapter, _, registry, _ = self.invoke(manifest(), ["--apply"])
