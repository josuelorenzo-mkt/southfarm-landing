"""tt-verify-live2.py -- SECOND LIVE TikTok publication test on the real phone.
...
Same agile TikTok verify as live1 (same user-defined exact timing as the
Instagram adapter, calibrated on fixtures only): fixed 20s propagation wait
after the "Video posted!" toast -> tab-cycle re-sync (Profile -> 3s ->
...
live2 change (cold-start race): right after the monkey launch the first
dump(s) can contain only SystemUI while the app is still rendering, which
made live runs abort PROFILE_TAB on a dump without the tab bar.  This
...
renders.  Everything else is unchanged from live1.
...
in diag/ with the ttverify2- prefix. Every adb invocation travels as
...
Run:  python tt-verify-live2.py    (cwd: worktree root)
"""
...
REMOTE_VIDEO = "/sdcard/Movies/SouthFarm/ttverify-live2.mp4"
DIAG = WORKTREE / "diag"
DIAG.mkdir(exist_ok=True)
LOG_PATH = DIAG / "ttverify2-live.log"
...
class EvidenceDevice:
    ...
    def save_snapshot(self, name: str, *, screenshot: bool) -> None:
        xml = self._raw_xml()
        if xml.strip():
            (DIAG / f"ttverify2-{name}.xml").write_bytes(xml)
        if screenshot:
            self._inner.screenshot(str(DIAG / f"ttverify2-{name}.png"))
        log(f"  evidence saved: ttverify2-{name}.xml" + (" + .png" if screenshot else ""))
...
def main() -> int:
    if LOG_PATH.exists():
        LOG_PATH.unlink()
    log(f"tt-verify-live2 start: serial={SERIAL} account=@{EXPECTED_ACCOUNT} video={LOCAL_VIDEO}")
    ...
    try:
        ...
        if identity is None:
            ...
            if evidence is not None:
                (DIAG / "ttverify2-unverified-evidence.json").write_text(
                    json.dumps(evidence, indent=2, ensure_ascii=False), encoding="utf-8"
                )
                log("  evidence saved: ttverify2-unverified-evidence.json (last dump attached)")
        else:
