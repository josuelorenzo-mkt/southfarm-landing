...
export class PublicationStore {
  ...
  private event(id: number, from: string | null, to: string | null, actor: PublicationActor, at: string, payload?: unknown): void {
    this.db.prepare('INSERT INTO publication_events (publication_job_id, from_status, to_status, current_step, actor_type, actor_id, payload, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
      .run(id, from, to, to, actor.type, actor.id, payload === undefined ? null : JSON.stringify(payload), at);
  }
  private transition(row: PublicationRow, to: PublicationStatus, actor: PublicationActor, at: string, payload?: unknown): PublicationRow {
    if (row.final_action_at && !['publishing', 'verifying', 'completed', 'review_required'].includes(to)) throw new Error('Cannot transition publication job after final action');
  ...
  rescheduleJob(id: number, scheduledFor: string, actor: PublicationActor): PublicationJobView { requireIso(scheduledFor, 'scheduledFor'); return this.transaction(() => { const row = this.row(id); if (row.status !== 'queued') throw new Error('Only queued publication jobs can be rescheduled'); this.db.prepare('UPDATE publication_jobs SET scheduled_for = ?, updated_at = ? WHERE id = ?').run(scheduledFor, scheduledFor, id); this.event(id, 'queued', 'queued', actor, scheduledFor, { scheduled_for: scheduledFor }); return this.getJob(id); }); }
  requestCancellation(id: number, actor: PublicationActor, at = new Date().toISOString()): PublicationJobView { return this.transaction(() => { const row = this.row(id); if (row.final_action_at) throw new Error('Cannot cancel publication job after final action'); if (row.status === 'queued') return publicationJobView(this.transition(row, 'cancelled', actor, at), this.db); if (!Object.prototype.hasOwnProperty.call(PUBLICATION_STATE_TRANSITIONS, row.status) || row.status === 'cancellation_requested') throw new Error(`Cannot cancel publication job in ${row.status}`); this.db.prepare('UPDATE publication_jobs SET cancel_requested_at = ? WHERE id = ?').run(at, id); return publicationJobView(this.transition(row, 'cancellation_requested', actor, at), this.db); }); }
  claimDueJob(worker: PublicationWorker, now: string): { claimed: boolean; job: PublicationJobView | null } { requireIso(now, 'now'); return this.transaction(() => { this.db.prepare('DELETE FROM device_automation_locks WHERE expires_at <= ?').run(now); const candidate = this.db.prepare(`SELECT job.* FROM publication_jobs job WHERE job.device_id = ? AND job.status = 'queued' AND job.scheduled_for <= ? AND NOT EXISTS (SELECT 1 FROM publication_jobs review WHERE review.social_account_id = job.social_account_id AND review.status = 'review_required') AND NOT EXISTS (SELECT 1 FROM task_runs run WHERE run.device_id = job.device_id AND ((run.status IN ('running', 'paused') AND (run.lease_expires_at IS NULL OR run.lease_expires_at > ?)) OR (run.status IN ('pending', 'overdue') AND (run.scheduled_for IS NULL OR run.scheduled_for <= ?) AND (run.expires_at IS NULL OR run.expires_at > ?)))) AND NOT EXISTS (SELECT 1 FROM device_automation_locks lock WHERE lock.device_id = job.device_id AND lock.expires_at > ?) ORDER BY job.scheduled_for, job.id LIMIT 1`).get(worker.deviceId, now, now, now, now, now) as PublicationRow | undefined; if (!candidate) return { claimed: false, job: null }; const expiresAt = isoAfter(now, Math.max(1, worker.leaseSeconds)); const claimToken = randomUUID(); this.db.prepare("UPDATE publication_jobs SET status = 'claimed', current_step = 'claimed', claimed_by = ?, claim_token = ?, claimed_at = ?, lease_expires_at = ?, last_heartbeat_at = ?, attempt_count = attempt_count + 1, updated_at = ? WHERE id = ?").run(worker.id, claimToken, now, expiresAt, now, now, candidate.id); this.db.prepare('INSERT INTO device_automation_locks (device_id, publication_job_id, worker_id, expires_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(device_id) DO UPDATE SET publication_job_id = excluded.publication_job_id, worker_id = excluded.worker_id, expires_at = excluded.expires_at, updated_at = excluded.updated_at').run(candidate.device_id, candidate.id, worker.id, expiresAt, now, now); this.event(candidate.id, 'queued', 'claimed', { type: 'worker', id: worker.id }, now); return { claimed: true, job: this.getJob(candidate.id) }; }); }
  heartbeat(id: number, worker: PublicationWorker, now: string): PublicationJobView { return this.transaction(() => { const row = this.row(id); if (row.claimed_by !== worker.id) throw new Error('Worker does not hold an active publication job'); const expiresAt = isoAfter(now, Math.max(1, worker.leaseSeconds)); const lockUpdate = this.db.prepare('UPDATE device_automation_locks SET expires_at = ?, updated_at = ? WHERE device_id = ? AND publication_job_id = ? AND worker_id = ? AND expires_at > ?').run(expiresAt, now, row.device_id, id, worker.id, now); if (lockUpdate.changes !== 1) throw new Error('Worker does not hold a live device automation lock'); const jobUpdate = this.db.prepare('UPDATE publication_jobs SET lease_expires_at = ?, last_heartbeat_at = ?, updated_at = ? WHERE id = ? AND claimed_by = ?').run(expiresAt, now, now, id, worker.id); if (jobUpdate.changes !== 1) throw new Error('Worker no longer owns publication job'); return this.getJob(id); }); }
  checkpoint(id: number, worker: PublicationWorker, now: string, options: { step: PublicationStatus; progressPercent: number; finalAction?: boolean }): PublicationJobView { return this.transaction(() => { let row = this.row(id); if (row.claimed_by !== worker.id) throw new Error('Worker does not hold this publication job'); if (!Number.isInteger(options.progressPercent) || options.progressPercent < 0 || options.progressPercent > 100) throw new Error('progressPercent must be between 0 and 100'); if (options.finalAction && options.step !== 'publishing') throw new Error('finalAction requires publishing step'); row = this.transition(row, options.step, { type: 'worker', id: worker.id }, now, { progress_percent: options.progressPercent }); this.db.prepare('UPDATE publication_jobs SET progress_percent = ?, final_action_at = CASE WHEN ? THEN COALESCE(final_action_at, ?) ELSE final_action_at END, updated_at = ? WHERE id = ?').run(options.progressPercent, options.finalAction ? 1 : 0, now, now, id); return this.getJob(id); }); }
  finish(id: number, worker: PublicationWorker, target: Extract<PublicationStatus, 'completed' | 'cancelled' | 'failed' | 'review_required'>, now: string, actor: PublicationActor = { type: 'worker', id: worker.id }): PublicationJobView { return this.transaction(() => { let row = this.row(id); if (row.claimed_by !== worker.id) throw new Error('Worker does not hold this publication job'); if (row.final_action_at && !['completed', 'review_required'].includes(target)) throw new Error('Cannot finish publication job that changed after final action'); row = this.transition(row, target, actor, now); this.db.prepare('UPDATE publication_jobs SET completed_at = ?, updated_at = ? WHERE id = ?').run(now, now, id); this.db.prepare('DELETE FROM device_automation_locks WHERE device_id = ? AND publication_job_id = ?').run(row.device_id, id); return publicationJobView(row, this.db); }); }
}
