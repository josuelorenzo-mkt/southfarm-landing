 ...
 */
export function chooseDailyTargetSeconds(
  mode: SchedulerMode = 'random',
  random: () => number = Math.random,
  fixedSeconds = DEFAULT_FIXED_WARMUP_SECONDS,
): number {
  if (mode === 'fixed') {
    return Math.min(
      DAILY_MAX_WARMUP_SECONDS,
      Math.max(DAILY_MIN_WARMUP_SECONDS, Math.round(fixedSeconds / 60) * 60),
    );
  }
...
export function chooseSessionCount(
  random: () => number = Math.random,
  fixedCount?: number,
): 2 | 3 {
  if (fixedCount === 2 || fixedCount === 3) return fixedCount;
 ...
 */
export function splitWarmupDurationSeconds(
  totalSeconds: number,
  sessionCount: 2 | 3,
  random: () => number = Math.random,
): number[] {
...
export function addHoursIso(value: string, hours: number): string {
...
export function overdueAtIso(scheduledFor: string): string {
  return addHoursIso(scheduledFor, 2);
}

export function expiresAtIso(scheduledFor: string): string {
...
export function localDateTimeToIso(
  ...
  const partValue = (type: string): number => Number(
    localizedParts.find((part) => part.type === type)?.value || 0,
  );
...
export function isTaskDue(scheduledFor: unknown, now = new Date()): boolean {
