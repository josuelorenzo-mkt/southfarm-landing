...
function migrationAlreadyApplied(db: SqliteDatabase, version: number): boolean {
  return Boolean(db.prepare('SELECT version FROM schema_migrations WHERE version = ?').get(version));
}

function markMigrationApplied(db: SqliteDatabase, version: number): void {
  db.prepare(
 ...
 */
export function applyClusterMigrations(db: SqliteDatabase): void {
  db.exec(`
  ...
  if (migrationAlreadyApplied(db, 3)) return;
