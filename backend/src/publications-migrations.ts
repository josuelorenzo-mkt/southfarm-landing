...
function addColumnIfMissing(db: SqliteDatabase, table: string, name: string, definition: string): void {
...
export function applyPublicationMigrations(db: SqliteDatabase): void {
  db.exec(`
  ...
  ] as const) addColumnIfMissing(db, 'publication_jobs', name, definition);
