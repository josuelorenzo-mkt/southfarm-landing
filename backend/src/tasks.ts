...
tasksRouter.post('/run', (req: Request, res: Response) => {
  ...
  const device: any = db.prepare('SELECT * FROM devices WHERE id = ? AND user_id = ?').get(device_id, userId);
  if (!device) {
  ...
  const template: any = db.prepare('SELECT * FROM task_templates WHERE task_type = ?').get(task_type);
  if (!template) {
...
tasksRouter.get('/runs', (req: Request, res: Response) => {
  const runs = db.prepare(
    'SELECT tr.*, d.device_name FROM task_runs tr JOIN devices d ON tr.device_id = d.id WHERE tr.user_id = ? ORDER BY tr.created_at DESC'
