...
function insertRoutineTask(
  ...
  opts: {
    ...
    clusterName: string;
    priority: number;
    // Si la reserva tuvo que correr la tarea al próximo hueco, el horario
    // original pedido (para auditoría en task_events).
...
function generateWarmupDay(
  ...
  for (const account of accounts) {
    ...
    for (const slot of slots) {
      ...
      if (!reservation.ok) {
        console.warn(
          `[ActivityPlanner] Slot sin hueco ese día: cluster ${cluster.id}, `
...
function generateScanDay(
  ...
  for (const account of accounts) {
    ...
    for (const time of hours) {
      ...
      if (!reservation.ok) {
        console.warn(
          `[ActivityPlanner] Scan sin hueco ese día: cluster ${cluster.id}, `
          + `device ${account.device_id}, ${scheduledFor} (${reservation.reason}).`,
        );
        continue;
      }
  ...
  app.get('/api/planner/day', deps.auth, (req: any, res) => {
    try {
      ...
      const rows = db.prepare(`
        ...
        ORDER BY COALESCE(tr.scheduled_for, tr.created_at) ASC, tr.id ASC
      `).all(...(validClusterId !== null ? [req.user.workspaceId, validClusterId] : [req.user.workspaceId])) as any[];
      ...
      if (validClusterId !== null) {
        ...
        ).get(validClusterId, req.user.workspaceId) as any;
        if (!clusterExists) {
