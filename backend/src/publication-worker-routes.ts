...
export function registerPublicationWorkerRoutes({ app, db, store, mediaRoot, workerTokenHash, onlineWindowSeconds = 90 }: { app: Express; db: SqliteDatabase; store: PublicationStore; mediaRoot: string; workerTokenHash: Buffer | string; onlineWindowSeconds?: number }): void {
  ...
  app.post('/api/publication-worker/claim', authenticate, (req, res) => {
    ...
    try {
      ...
      const media: any = db.prepare(`SELECT id, size_bytes, LOWER(sha256) AS sha256, mime_type, file_extension FROM publication_media
        WHERE id = ? AND workspace_id = ? AND upload_status = 'stored'`).get(job.media_id, job.workspace_id);
      if (!media || Number(media.id) !== Number(job.media_id)) return res.status(409).json({ error_code: 'WORKER_CLAIM_INVALID', error: 'Claimed publication media is unavailable' });
      job.media = media;
