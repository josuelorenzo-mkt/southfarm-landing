...
export function registerPublicationWorkerRoutes({ app, db, store, mediaRoot, workerTokenHash, onlineWindowSeconds = 90 }: { app: Express; db: SqliteDatabase; store: PublicationStore; mediaRoot: string; workerTokenHash: Buffer | string; onlineWindowSeconds?: number }): void {
  ...
  const authorizationPayload = (token: unknown, consume: boolean, workerId: string | null, deviceId: number | null): { nonce: string; payload: any } | null => {
    ...
    if (!workerId || !deviceId) return null;
    const row: any = db.prepare(`SELECT auth.payload, auth.expires_at, auth.consumed_at FROM publication_cleanup_authorizations auth
    ...
    if (consume && db.prepare('UPDATE publication_cleanup_authorizations SET consumed_at = ? WHERE nonce = ? AND worker_id = ? AND device_id = ? AND consumed_at IS NULL AND expires_at > ?').run(new Date().toISOString(), nonce, workerId, deviceId, new Date().toISOString()).changes !== 1) return null;
  ...
  app.get('/api/publication-worker/devices/:id/availability', authenticate, (req, res) => {
    ...
    const now = new Date().toISOString(); const device: any = db.prepare('SELECT id, device_id, device_name, lifecycle_status, last_seen_at, workspace_id FROM devices WHERE id = ?').get(deviceId);
    ...
    const control: any = db.prepare('SELECT scheduler_mode, queue_paused FROM workspace_controls WHERE workspace_id = ?').get(device.workspace_id);
    ...
    const task: any = db.prepare(`SELECT id FROM task_runs WHERE device_id = ? AND ((status IN ('running', 'paused') AND (lease_expires_at IS NULL OR lease_expires_at > ?)) OR (status IN ('pending', 'overdue') AND (scheduled_for IS NULL OR scheduled_for <= ?) AND (expires_at IS NULL OR expires_at > ?) ${routineGate})) LIMIT 1`).get(deviceId, now, now, now);
