...
const configuredPrimarySecret = String(process.env.JWT_SECRET || process.env.SOUTHFARM_JWT_SECRET || '').trim();
export const JWT_SECRET = configuredPrimarySecret || DEFAULT_JWT_SECRET;
const configuredLegacySecrets = String(process.env.SOUTHFARM_JWT_LEGACY_SECRETS || '')
    .split(',')
...
const configuredAccessTokenTtl = String(process.env.SOUTHFARM_ACCESS_TOKEN_TTL || '15m').trim();
...
export const ACCESS_TOKEN_TTL = configuredAccessTokenTtl;
export function signSouthFarmJwt(userId) {
    return jwt.sign({ userId }, JWT_SECRET, { expiresIn: ACCESS_TOKEN_TTL });
}
export function verifySouthFarmJwt(token) {
    let lastError;
