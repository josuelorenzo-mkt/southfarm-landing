...
export class PublicationStore {
    ...
    transition(row, to, actor, at, payload) {
        if (row.final_action_at && !['publishing', 'verifying', 'completed', 'review_required'].includes(to))
            throw new Error('Cannot transition publication job after final action');
    ...
    rescheduleJob(id, scheduledFor, actor) { requireIso(scheduledFor, 'scheduledFor'); return this.transaction(() => { const row = this.row(id); if (row.status !== 'queued')
        throw new Error('Only queued publication jobs can be rescheduled'); this.db.prepare('UPDATE publication_jobs SET scheduled_for = ?, updated_at = ? WHERE id = ?').run(scheduledFor, scheduledFor, id); this.event(id, 'queued', 'queued', actor, scheduledFor, { scheduled_for: scheduledFor }); return this.getJob(id); }); }
    requestCancellation(id, actor, at = new Date().toISOString()) { return this.transaction(() => { const row = this.row(id); if (row.final_action_at)
        throw new Error('Cannot cancel publication job after final action'); if (row.status === 'queued')
        ...
        throw new Error(`Cannot cancel publication job in ${row.status}`); this.db.prepare('UPDATE publication_jobs SET cancel_requested_at = ? WHERE id = ?').run(at, id); return publicationJobView(this.transition(row, 'cancellation_requested', actor, at), this.db); }); }
    ...
    heartbeat(id, worker, now) { return this.transaction(() => { const row = this.row(id); if (row.claimed_by !== worker.id)
        throw new Error('Worker does not hold an active publication job'); const expiresAt = isoAfter(now, Math.max(1, worker.leaseSeconds)); const lockUpdate = this.db.prepare('UPDATE device_automation_locks SET expires_at = ?, updated_at = ? WHERE device_id = ? AND publication_job_id = ? AND worker_id = ? AND expires_at > ?').run(expiresAt, now, row.device_id, id, worker.id, now); if (lockUpdate.changes !== 1)
        throw new Error('Worker does not hold a live device automation lock'); const jobUpdate = this.db.prepare('UPDATE publication_jobs SET lease_expires_at = ?, last_heartbeat_at = ?, updated_at = ? WHERE id = ? AND claimed_by = ?').run(expiresAt, now, now, id, worker.id); if (jobUpdate.changes !== 1)
        throw new Error('Worker no longer owns publication job'); return this.getJob(id); }); }
    checkpoint(id, worker, now, options) { return this.transaction(() => { let row = this.row(id); if (row.claimed_by !== worker.id)
        throw new Error('Worker does not hold this publication job'); if (!Number.isInteger(options.progressPercent) || options.progressPercent < 0 || options.progressPercent > 100)
    ...
    finish(id, worker, target, now, actor = { type: 'worker', id: worker.id }) { return this.transaction(() => { let row = this.row(id); if (row.claimed_by !== worker.id)
        throw new Error('Worker does not hold this publication job'); if (row.final_action_at && !['completed', 'review_required'].includes(target))
        throw new Error('Cannot finish publication job that changed after final action'); row = this.transition(row, target, actor, now); this.db.prepare('UPDATE publication_jobs SET completed_at = ?, updated_at = ? WHERE id = ?').run(now, now, id); this.db.prepare('DELETE FROM device_automation_locks WHERE device_id = ? AND publication_job_id = ?').run(row.device_id, id); return publicationJobView(row, this.db); }); }
}
