...
export const PLANNER_VIDEO_MIME_EXTENSIONS = {
    'video/mp4': 'mp4',
    'video/quicktime': 'mov',
    'video/webm': 'webm',
};
...
export async function createPlannerPublicationJobs(opts) {
    ...
    const result = { publicationIds: [], skipped: [] };
    if (!extension || !PLANNER_VIDEO_MIME_EXTENSIONS[mimeType]) {
        // Unsupported mime: every account is skipped for the same reason.
        for (const entry of accounts)
            result.skipped.push({ account: entry.account.username, reason: `Unsupported video mime ${mimeType || '(none)'} — use mp4, mov or webm` });
        return result;
    }
    ...
    try {
        ...
        for (const entry of accounts)
            result.skipped.push({ account: entry.account.username, reason: 'Video metadata could not be inspected (invalid or truncated file)' });
        return result;
    }
    ...
    for (const entry of accounts) {
        ...
        try {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'queued', 'queued', ?, ?, ?, 'planner', ?, ?, ?, ?, ?)`)
                    .run(workspaceId, userId, account.device_id, account.id, mediaId, input.platform, input.caption, input.wordCount, scheduledFor, clusterId, clusterName, clusterAssetId, String(userId), JSON.stringify({ id: Number(account.id), username: String(account.username), display_name: String(account.display_name || account.username), platform: String(account.platform) }), JSON.stringify(device ? { id: Number(device.id), device_id: String(device.device_id) } : { id: Number(account.device_id), device_id: '' }), createdAtIso, createdAtIso);
          ...
          VALUES (?, NULL, 'queued', 'queued', 'planner', ?, ?)`).run(jobId, String(userId), createdAtIso);
            ...
            if (mediaId) {
                try {
                    db.prepare('DELETE FROM publication_media WHERE id = ? AND upload_status = ?').run(mediaId, 'staging');
                }
                catch { /* best effort */ }
                if (mediaFile) {
                    try {
                        fs.rmSync(path.join(mediaRoot, mediaFile), { force: true });
                    }
                    catch { /* best effort */ }
                }
            }
