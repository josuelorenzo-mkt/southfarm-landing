...
const AVATAR_PLATFORMS = ['instagram', 'tiktok', 'youtube'];
...
export function sanitizeAvatarUsername(username) {
    return String(username || '').replace(/[^a-zA-Z0-9._-]/g, '_');
}
export function avatarFileName(platform, username) {
    return `${platform}_${sanitizeAvatarUsername(username)}.jpg`;
}
export function avatarFilePath(platform, username) {
...
export function avatarRelativeUrl(platform, username) {
    return `/api/avatars/${avatarFileName(platform, username)}`;
}
function profilePageUrl(platform, username) {
...
function httpsGet(url, maxBytes, extraHeaders) {
...
function extractYouTubeAvatarUrl(html) {
...
function unescapeEmbeddedUrl(url) {
...
function extractInstagramAvatarFromBody(rawBody) {
    ...
    for (const pattern of [
        ...
        if (match?.[1])
            return unescapeEmbeddedUrl(match[1]);
    }
    return '';
}
...
async function fetchInstagramWebProfileUrl(username) {
    ...
    if (!response || response.status !== 200 || !response.body.length)
        return '';
...
function extractTikTokAvatarUrl(html) {
    for (const pattern of [
        /"avatar[^"]*"\s*:\s*"(https:\\?\/\\?\/[^"]+)"/,
        /"(https:\\?\/\\?\/p16-sign[^"]+)"/,
    ]) {
...
async function fetchTikTokProfilePicUrl(username) {
    ...
    if (botResponse && botResponse.status === 200 && botResponse.body.length) {
        ...
        if (botOgImageUrl && /^https?:\/\//i.test(botOgImageUrl))
            return botOgImageUrl;
...
async function fetchSourceProfilePicUrl(platform, username) {
    if (platform === 'instagram') {
...
export function fetchInstagramProfilePicUrl(username) {
...
export async function ensureAvatarStored(platform, username) {
    const normalizedPlatform = String(platform || '').toLowerCase();
    if (!AVATAR_PLATFORMS.includes(normalizedPlatform))
        return '';
