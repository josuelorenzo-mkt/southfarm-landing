...
function hasActiveTaskForSlot(deps, deviceId, taskType, scheduledFor) {
...
function publicationCoarseStatus(jobStatus) {
    ...
    if (PUBLICATION_RUNNING_STEPS.has(jobStatus))
        return 'running';
    return 'queued';
}
...
function plannerPublicationView(deps, row) {
    let snapshot = {};
    try {
        snapshot = row.account_snapshot ? JSON.parse(row.account_snapshot) : {};
    }
    catch { /* keep empty */ }
    return {
        id: Number(row.id),
...
function weekPublications(deps, workspaceId, clusterId, weekStart, weekEnd) {
    const rows = deps.db.prepare(`
...
export function registerActivityPlanner(app, deps) {
    ...
    app.post('/api/clusters/:id/publish', deps.auth, deps.requireRole('owner', 'admin', 'operator'), uploadClusterAsset, async (req, res) => {
        try {
            const cluster = clusterById(req.user.workspaceId, req.params.id);
            if (!cluster)
                return res.status(404).json({ error: 'Cluster not found' });
            ...
            const response = { created: bridge.publicationIds.length, assetId, publicationIds: bridge.publicationIds };
            if (bridge.skipped.length)
                response.skipped = bridge.skipped;
    ...
    // preview.
    app.get('/api/planner/publications', deps.auth, (req, res) => {
        try {
            const rows = db.prepare(`
