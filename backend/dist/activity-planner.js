...
import { BUENOS_AIRES_TIMEZONE, localDateTimeToIso, overdueAtIso, expiresAtIso, splitWarmupDurationSeconds, } from './scheduler.js';
...
function parseRoutineConfig(routineType, raw) {
    ...
    const config = { ...DEFAULT_ROUTINE_CONFIGS[routineType] };
...
function generateWarmupDay(deps, workspaceId, ownerId, cluster, routine, accounts, dateKey, todayKey) {
    ...
    for (const account of accounts) {
        ...
        const durations = splitWarmupDurationSeconds(minMinutes * 60, sessionCount, Math.random);
    ...
    if (routines.length > 0 && routines.every((routine) => routine.status === 'paused')) {
        ...
        for (let i = 0; i < todayIdx; i += 1) {
            ...
            if (approvedPublish) {
                ...
                if (publishingDaysFor(cluster.id, config.postsPerWeek).includes(i)
...
function computeSummary(deps, items) {
    let tasksTotal = 0;
    let tasksRunning = 0;
    let tasksQueued = 0;
    let publishTotal = 0;
...
export function registerActivityPlanner(app, deps) {
    ...
    // ── 7. Cluster publish ──
    app.post('/api/clusters/:id/publish', deps.auth, deps.requireRole('owner', 'admin', 'operator'), (req, res) => {
        const cluster = clusterById(req.user.workspaceId, req.params.id);
        if (!cluster)
            return res.status(404).json({ error: 'Cluster not found' });
        const blocked = workspaceBlockedMessage(req.user.workspaceId);
