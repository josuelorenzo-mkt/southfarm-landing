...
export function registerPublicationWorkerRoutes({ app, db, store, mediaRoot, workerTokenHash }) {
    ...
    const owner = (req, id) => {
        ...
        if (!workerId || !token)
            return null;
        const row = db.prepare('SELECT device_id FROM publication_jobs WHERE id = ? AND claimed_by = ? AND claim_token = ? AND lease_expires_at > ?').get(id, workerId, token, new Date().toISOString());
    ...
    app.post('/api/publication-worker/jobs/:id/heartbeat', authenticate, (req, res) => {
        ...
        const worker = id ? owner(req, id) : null;
        if (!id || !worker)
    ...
    app.post('/api/publication-worker/jobs/:id/checkpoint', authenticate, (req, res) => {
        ...
        const finalAction = Boolean(req.body?.final_action);
        if (!id || !worker)
            return conflict(res, new Error('Worker claim is invalid or expired'));
        if (!step || !CHECKPOINTS.has(step) || !Number.isInteger(progress))
            return res.status(400).json({ error_code: 'CHECKPOINT_INVALID', error: 'step and progress_percent are invalid' });
        try {
            const job = store.checkpoint(id, worker, new Date().toISOString(), { step: step, progressPercent: progress, finalAction });
            const evidence = safeJson(req.body?.evidence);
            if (evidence)
                db.prepare('UPDATE publication_events SET payload = ? WHERE id = (SELECT MAX(id) FROM publication_events WHERE publication_job_id = ?)').run(evidence, id);
    ...
    app.post('/api/publication-worker/jobs/:id/finish', authenticate, (req, res) => {
        ...
        try {
            const current = db.prepare('SELECT final_action_at FROM publication_jobs WHERE id = ?').get(id);
            if (current?.final_action_at && (target === 'failed' || target === 'cancelled'))
                return res.status(409).json({ error_code: 'FINAL_ACTION_UNCERTAIN', error: 'Final action requires review_required or completed' });
            ...
            const job = store.finish(id, worker, target, new Date().toISOString());
    ...
    app.get('/api/publication-worker/media/:id', authenticate, (req, res) => {
        ...
        if (!mediaId || !workerId || !token)
            return conflict(res, new Error('Worker claim is invalid or expired'));
        const row = db.prepare(`SELECT media.* FROM publication_media media JOIN publication_jobs job ON job.media_id = media.id WHERE media.id = ? AND job.claimed_by = ? AND job.claim_token = ? AND job.lease_expires_at > ? AND media.upload_status = 'stored'`).get(mediaId, workerId, token, new Date().toISOString());
        if (!row)
    ...
    app.get('/api/publication-worker/devices/:id/availability', authenticate, (req, res) => {
        ...
        const task = db.prepare("SELECT id FROM task_runs WHERE device_id = ? AND status IN ('running', 'paused') AND (lease_expires_at IS NULL OR lease_expires_at > ?) LIMIT 1").get(deviceId, now);
