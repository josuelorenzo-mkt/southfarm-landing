...
app.post('/api/devices/register', auth, (req, res) => {
    ...
    const existing = db.prepare('SELECT id FROM devices WHERE user_id = ? AND device_id = ? ORDER BY id DESC LIMIT 1').get(req.user.userId, device_id);
    if (existing) {
...
app.post('/api/tasks/run', auth, (req, res) => {
...
app.patch('/api/tasks/runs/:id', auth, (req, res) => {
    ...
    const r = db.prepare('SELECT * FROM task_runs WHERE id = ? AND user_id = ?').get(req.params.id, req.user.userId);
    if (!r)
...
app.post('/api/ig-accounts', auth, async (req, res) => {
    ...
    if (typeof device_id === 'string') {
        const device = db.prepare('SELECT id FROM devices WHERE user_id = ? AND device_id = ? ORDER BY id DESC LIMIT 1').get(req.user.userId, device_id);
...
app.get('/api/ig-accounts', auth, (req, res) => {
    ...
    if (deviceStrId) {
        ...
        let numericId = parseInt(deviceStrId, 10);
        if (isNaN(numericId)) {
            const device = db.prepare('SELECT id FROM devices WHERE user_id = ? AND device_id = ? ORDER BY id DESC LIMIT 1').get(req.user.userId, deviceStrId);
...
app.post('/api/warmup-sessions', auth, (req, res) => {
    ...
    VALUES (?, 'warmup_ig', ?, ?, ?, ?, ?)
  `).run(req.user.userId, status || 'completed', JSON.stringify({ account, duration_minutes }), JSON.stringify({ reels_viewed, likes, saves, elapsed_sec }), timestamp || new Date().toISOString(), new Date().toISOString());
