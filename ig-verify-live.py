"""ig-verify-live.py -- one-shot LIVE Instagram publication test on the real phone.
...
Run:  python ig-verify-live.py    (cwd: worktree root)
"""
...
LOCAL_VIDEO = r"C:\Users\josu_\Downloads\Videos to test\MP-V-2.mp4"
REMOTE_VIDEO = "/sdcard/Movies/SouthFarm/igverify-live.mp4"
DIAG = WORKTREE / "diag"
DIAG.mkdir(exist_ok=True)
LOG_PATH = DIAG / "igverify-live.log"

# The adapter hard-caps captions at 10 words (validate_caption); 10 it is.
CAPTION = "Stay hungry, stay humble, and keep moving forward every day."
...
class EvidenceDevice:
    ...
    def save_snapshot(self, name: str, *, screenshot: bool) -> None:
        xml = self._raw_xml()
        if xml.strip():
            (DIAG / f"igverify-{name}.xml").write_bytes(xml)
        if screenshot:
            self._inner.screenshot(str(DIAG / f"igverify-{name}.png"))
        log(f"  evidence saved: igverify-{name}.xml" + (" + .png" if screenshot else ""))
...
def media_metadata(path: str) -> dict:
    ...
    return {
        ...
        "mime_type": "video/mp4",
        "file_extension": "mp4",
        "duration_seconds": 27,  # ffprobe: 27.005011s -> gallery label "0:27"
        "width": 2160,
        "height": 3840,
        "video_codec": "hevc",
        "audio_codec": "aac",
...
def main() -> int:
    if LOG_PATH.exists():
        LOG_PATH.unlink()
    log(f"ig-verify-live start: serial={SERIAL} account={EXPECTED_ACCOUNT} video={LOCAL_VIDEO}")
