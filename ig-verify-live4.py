"""ig-verify-live4.py -- fourth LIVE Instagram publication test on the real phone.
...
(no backend, no runner). Mirrors runner.py's invocation order:
    ...
    ensure_accessibility_healthy (pre-check, as runner.py:70)
    -> prepare -> push+scan media -> publish(checkpoint) -> verify

This run exercises the NEW verify code path in the worker:
- the accessibility pre-check with single auto-repair (ensure_accessibility_healthy);
- the agile post-Share verify: fixed 20s propagation wait -> tab-cycle re-sync
  (Profile -> 3s -> Home -> 3s -> Profile) + fixed-coordinate Bezier
  pull-to-refresh -> composite delta+identity check, then 20s -> refresh ->
  check, then 10s -> refresh -> check; when a check detects delta, the grid
  is recomposed with a full tab cycle at 1s settles BEFORE the identity tap
  (swipe-only refresh updates the header count but keeps the old tiles on
  this build, live3); after 3 checks without positive evidence the adapter
  returns None (worker-local `unverified`, never republished).

Media: "0730 MA-V-4.mp4" (HEVC 1080x1920, 16.65s -> gallery label "0:17",
probe-proven by diag/igverify4-probe-label.py). MA-V-3 was published by
ig-verify-live3.py and is NOT reused.
...
Run:  python ig-verify-live4.py    (cwd: worktree root)
"""
...
REMOTE_VIDEO = "/sdcard/Movies/SouthFarm/igverify-live4.mp4"
DIAG = WORKTREE / "diag"
DIAG.mkdir(exist_ok=True)
LOG_PATH = DIAG / "igverify4-live.log"

# The adapter hard-caps captions at 10 words (validate_caption); 9 it is.
# New caption (mindset/motivational), distinct from the previous runs'
# captions (ig-verify-live, ig-verify-live2, ig-verify-live3).
...
class EvidenceDevice:
    ...
    def dump_ui(self):
        ...
        if self.phase == "verify":
            log(f"  [verify dump #{self.dumps}] wall={wall()} {self._since_share()}")
    ...
    def save_snapshot(self, name: str, *, screenshot: bool) -> None:
        xml = self._raw_xml()
        if xml.strip():
            (DIAG / f"igverify4-{name}.xml").write_bytes(xml)
        if screenshot:
            self._inner.screenshot(str(DIAG / f"igverify4-{name}.png"))
        log(f"  evidence saved: igverify4-{name}.xml" + (" + .png" if screenshot else ""))
...
def media_metadata(path: str) -> dict:
    ...
    return {
        ...
        "mime_type": "video/mp4",
        "file_extension": "mp4",
        "duration_seconds": 17,  # ffprobe: 16.648s -> gallery label "0:17" (probe-proven)
        "width": 1080,
        "height": 1920,
        "video_codec": "hevc",
        "audio_codec": "aac",
...
def main() -> int:
    if LOG_PATH.exists():
        LOG_PATH.unlink()
    log(f"ig-verify-live4 start: serial={SERIAL} account={EXPECTED_ACCOUNT} video={LOCAL_VIDEO}")
    ...
    try:
        # 0) accessibility pre-check with single auto-repair (runner.py:70 path)
        device.phase = "precheck"
        t0 = time.monotonic()
        if device._service_answers():
            log(f"PRE-CHECK OK wall={wall()}: accessibility service answered a fresh dump on first try ({time.monotonic() - t0:.1f}s)")
        else:
            log(f"PRE-CHECK wall={wall()}: service did NOT answer the first fresh dump ({time.monotonic() - t0:.1f}s); invoking ensure_accessibility_healthy (one repair + re-verify)")
            t1 = time.monotonic()
            device.ensure_accessibility_healthy()
            log(f"PRE-CHECK REPAIRED wall={wall()}: service answers again after repair ({time.monotonic() - t1:.1f}s)")
        ...
        if identity is None:
            ...
            if evidence is not None:
                (DIAG / "igverify4-unverified-evidence.json").write_text(
                    json.dumps(evidence, indent=2, ensure_ascii=False), encoding="utf-8"
                )
                log("  evidence saved: igverify4-unverified-evidence.json (last dump attached)")
        else:
