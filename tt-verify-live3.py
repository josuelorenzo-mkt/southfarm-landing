"""tt-verify-live3.py -- THIRD LIVE TikTok publication test on the real phone.
...
in diag/ with the ttverify3- prefix. Every adb invocation travels as
...
Run:  python tt-verify-live3.py    (cwd: worktree root)
Exit codes: 0 = verified, 2 = UNVERIFIED (publication real, verification
pending -- evidence saved as ttverify3-unverified-evidence.json), 1 = aborted.
"""
...
REMOTE_VIDEO = "/sdcard/Movies/SouthFarm/ttverify-live3.mp4"
DIAG = WORKTREE / "diag"
DIAG.mkdir(exist_ok=True)
LOG_PATH = DIAG / "ttverify3-live.log"

# 8 words, within the 10-word cap.
CAPTION = "Your habits shape your future not your dreams"
...
class EvidenceDevice:
    ...
    def save_snapshot(self, name: str, *, screenshot: bool) -> None:
        xml = self._raw_xml()
        if xml.strip():
            (DIAG / f"ttverify3-{name}.xml").write_bytes(xml)
        if screenshot:
            self._inner.screenshot(str(DIAG / f"ttverify3-{name}.png"))
        log(f"  evidence saved: ttverify3-{name}.xml" + (" + .png" if screenshot else ""))
...
def main() -> int:
    if LOG_PATH.exists():
        LOG_PATH.unlink()
    log(f"tt-verify-live3 start: serial={SERIAL} account=@{EXPECTED_ACCOUNT} video={LOCAL_VIDEO}")
    ...
    try:
        ...
        if identity is PublicationStatus.UNVERIFIED:
            ...
            if evidence is not None:
                (DIAG / "ttverify3-unverified-evidence.json").write_text(
                    json.dumps(evidence, indent=2, ensure_ascii=False), encoding="utf-8"
                )
                log("  evidence saved: ttverify3-unverified-evidence.json (last dump attached)")
