"""ig-verify-live3.py -- third LIVE Instagram publication test on the real phone.
...
This run exercises the NEW code path just implemented in the worker:
- the accessibility pre-check with single auto-repair (ensure_accessibility_healthy);
- the agile post-Share verify: fixed 15s propagation wait -> tab-cycle re-sync
  (Profile -> 1s -> Home -> 1s -> Profile) + fixed-coordinate Bezier
  pull-to-refresh -> composite delta+identity check, then 8s -> refresh ->
  check, then 8s -> refresh -> check; after 3 checks without positive evidence
  the adapter returns None (worker-local `unverified`, never republished).

Media: "0730 MA-V-3.mp4" (HEVC 1080x1920, 15.37s -> gallery label "0:15").
MA-V-2 was already published by ig-verify-live2.py and is NOT reused.
...
Run:  python ig-verify-live3.py    (cwd: worktree root)
"""
...
REMOTE_VIDEO = "/sdcard/Movies/SouthFarm/igverify-live3.mp4"
DIAG = WORKTREE / "diag"
DIAG.mkdir(exist_ok=True)
LOG_PATH = DIAG / "igverify3-live.log"

# The adapter hard-caps captions at 10 words (validate_caption); 7 it is.
# New caption (mindset/motivational), distinct from the previous runs'
# "Stay hungry, stay humble..." (ig-verify-live), "Stay present, tomorrow is
# not promised" and "Consistency compounds quietly so show up and do the work"
# (ig-verify-live2).
CAPTION = "Discipline is choosing what you want most"
...
class EvidenceDevice:
    ...
    def save_snapshot(self, name: str, *, screenshot: bool) -> None:
        xml = self._raw_xml()
        if xml.strip():
            (DIAG / f"igverify3-{name}.xml").write_bytes(xml)
        if screenshot:
            self._inner.screenshot(str(DIAG / f"igverify3-{name}.png"))
        log(f"  evidence saved: igverify3-{name}.xml" + (" + .png" if screenshot else ""))
...
def media_metadata(path: str) -> dict:
    ...
    return {
        ...
        "mime_type": "video/mp4",
        "file_extension": "mp4",
        "duration_seconds": 15,  # ffprobe: 15.367s -> gallery label "0:15"
        "width": 1080,
        "height": 1920,
        "video_codec": "hevc",
        "audio_codec": "aac",
...
def main() -> int:
    if LOG_PATH.exists():
        LOG_PATH.unlink()
    log(f"ig-verify-live3 start: serial={SERIAL} account={EXPECTED_ACCOUNT} video={LOCAL_VIDEO}")
    ...
    try:
        ...
        # 4) verify: agile 15s -> tab-cycle+refresh -> check, 8s -> refresh -> check, 8s -> refresh -> check
        device.phase = "verify"
        started = time.monotonic()
        ...
        if identity is None:
            ...
            if evidence is not None:
                (DIAG / "igverify3-unverified-evidence.json").write_text(
                    json.dumps(evidence, indent=2, ensure_ascii=False), encoding="utf-8"
                )
                log("  evidence saved: igverify3-unverified-evidence.json (last dump attached)")
        else:
